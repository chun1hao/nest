...
export class UserService {
  ...
  async register(registerInfo: RegisterDto) {
    const code = await this.redisService.get(`captcha_${registerInfo.email}`);
    if (!code) {
      throw new HttpException('验证码失效', HttpStatus.BAD_REQUEST);
    }
    if (code != registerInfo.captcha) {
    ...
    newUser.password = md5(registerInfo.password);
    newUser.email = registerInfo.email;
    newUser.nickName = registerInfo.nickName;
  ...
  async login(userInfo: LoginDto, isAdmin: boolean = false) {
    ...
    if (findUser.password !== md5(userInfo.password)) {
  ...
  async updatePassword(passwordDto: UpdateUserPasswordDto) {
    ...
    if (passwordDto.email !== fundUser.email) {
      throw new HttpException('邮箱不正确', HttpStatus.BAD_REQUEST);
    }

    fundUser.password = md5(passwordDto.password);
